...
fn main() {
    tauri::Builder::default()
        ...
        .run(|app, event| match event {
            // Разрешение на баннеры спрашиваем здесь, когда цикл событий уже работает
            // и приложение для системы «запустилось» — так положено по модели AppKit.
            // Отказ «Notifications are not allowed» 03.09.2026 объяснялся не моментом
            // вызова (в `setup` результат был тем же), а сожжённым идентификатором
            // бандла и запуском из временного каталога — см. notify.rs и README.
            RunEvent::Ready => notify::request_authorization(),
