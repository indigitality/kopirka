...
# Уведомление отсюда больше не шлём. Раньше это был osascript, и macOS рисовала
# рядом с текстом иконку Script Editor, а не «Копирки»: уведомление принадлежит
# тому, кто его показал. Теперь про приехавшие файлы говорит само приложение —
# сервер ведёт журнал импорта (GET /api/events), а оболочка его опрашивает.
# Ошибки по-прежнему уходят в лог, а не на экран: папка работает в фоне.
