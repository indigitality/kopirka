 ...
 * 47 806 591 байт (Apple Silicon) и 50 401 883 (Intel) по выводу
 * `pack-downloads.sh` 11.09.2026 (0.3.0), отсюда «48–50 МБ».
...
export const PLATFORMS: Record<PlatformId, PlatformRelease> = {
  macos: {
    ...
    /** Вес архивов 0.3.0 — см. цифры в комментарии к `PLATFORMS` выше. */
    size: '48–50 МБ',
    builds: [
      {
        id: 'aarch64',
  ...
  windows: {
    ...
    arch: 'x64',
    /**
     * Вес архива 0.3.0 — 34 392 401 байт, из вывода `pack-downloads.sh`
     * (11.09.2026).
     */
    size: '34 МБ',
    builds: [
 ...
 *
 * Страница опубликована в веб 11.09.2026 (Сергей включил «Опубликовать»):
 * по этому адресу она открывается без входа в Notion, проверено анонимным
 * браузером. Если публикацию снять, ссылку гасят привычным способом — `'#'`,
 * и `isLive` ниже уберёт её и в подвале, и в блоке «Скачать». `confirmed`.
 */
