...
export function DetailView() {
  ...
  useEffect(() => {
    if (openFileId !== null && file === null && !library.loading) close();
    ...
    <AnimatePresence>
      ...
      >
        <div className="pointer-events-none flex h-full w-full gap-4 p-4">
          ...
          {/* Правая панель */}
          <div className="pointer-events-auto relative flex h-full shrink-0 items-stretch">
            <IconButton
              label="Закрыть просмотр"
              onClick={close}
              className="absolute -top-1 -left-11 z-10 bg-surface-overlay"
            >
            ...
            <DetailPanel
              ...
              onCopy={() => {
                void api
                  .copyFile(file.id)
                  .then(() => toast({ title: 'Скопировано в буфер' }))
              ...
              onRestore={() => {
                void library
                  ...
                  .then(() => {
                    close();
                    toast({ title: 'Файл вернулся в библиотеку' });
                  })
