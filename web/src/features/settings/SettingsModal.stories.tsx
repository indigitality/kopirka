 ...
 * обратные слэши. Мок значения, не платформы: подсказка «По умолчанию…» и
 * «Перенести библиотеку… в Finder» под полем читают реальную платформу окна
 * (`platformStrings`), которой в браузерной витрине нет, — здесь они по-прежнему
 * говорят про Finder, и это ожидаемо для истории, которая проверяет только
 * длинный путь с обратными слэшами в поле, а не платформенный текст вокруг него.
 */
export const ПутьWindows: Story = {
  name: 'Путь Windows',
  args: {
    settings: { ...settings, libraryPath: String.raw`C:\Users\sergey\Pictures\Копирка` },
  },
};
